...
Entity.prototype.move_blocking = function(dx, dy, slide, solid_test) {
    let result = this.project_blocking(dx, dy, slide, solid_test);
    this.x = result.x;
    this.y = result.y;
    return result.blocked;
}
// Begin horror.
Entity.prototype.project_blocking = function(dx, dy, slide, solid_test) {
    ...
    // Due to certain rounding errors between project/move_blocking
    // calls, an entity may sometimes move through and into a wall.
    // To solve this, we back up a bit before moving on.
    const clipthreshold = 0.0625;
    let tdx = dx * this.world.time_delta_sec; // Target
    let tdy = dy * this.world.time_delta_sec;
    let distance = Math.sqrt(tdx**2+tdy**2) + clipthreshold;
    let speed = Math.sqrt(dx**2+dy**2);
    let udx = dx / speed; // Unit
    let udy = dy / speed;
    let xclipthreshold = udx * clipthreshold;
    let yclipthreshold = udy * clipthreshold;
    let bx = Math.sign(udx) * this.radius; // Front offset
    let by = Math.sign(udy) * this.radius;
    let x = this.x - xclipthreshold;
    let y = this.y - yclipthreshold;
    let fx = x + bx; // Front
    let fy = y + by;
    ...
    x = fx - bx; // This can have problematic rounding errors. (fixed with clipthreshold)
    y = fy - by;
    return {
        x: x,
        y: y,
