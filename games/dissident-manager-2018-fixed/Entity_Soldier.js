...
Entity_Soldier.prototype.damage = function(damage, actor) {
    return false;
}
...
Entity_Soldier.prototype.think = function() {
    ...
    if( mind.action !== null ) {
        ...
        if( ( mind.action.walking === false ) && ( mind.action.want_walk === true ) &&
            !this.project_blocking(CLEAR_WALK_DIST * Math.cos(this.rotation), CLEAR_WALK_DIST * Math.sin(this.rotation)).blocked )
        {
    ...
    if( this.aiming ) {
        ...
        if( mind.action === null ) {
            ...
            if( walk && this.project_blocking(CLEAR_WALK_DIST * Math.cos(this.rotation), CLEAR_WALK_DIST * Math.sin(this.rotation)).blocked ) {
                walk = false;
                want_walk = true;
            }
...
Entity_Soldier.prototype.update = function() {
    ...
    if (this.firing) {
        ...
        if (world.time_tick(100)) {
            ...
            let x = this.x;
            let y = this.y;
            let ox = Math.cos(rotation)*0.5;
            let oy = Math.sin(rotation)*0.5;
